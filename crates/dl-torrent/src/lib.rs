...
/// The bridge's routes, ready to serve or to mount inside another server.
///
/// Public because the desktop build runs the web app, the relay and this in one process
/// on one port: same origin, so nothing has to cross CORS, mixed content or private
/// network rules — and nothing has to be started by hand.
pub async fn router(output_folder: &std::path::Path) -> anyhow::Result<Router> {
    let session = Session::new(output_folder.to_path_buf()).await?;
    let bridge = Bridge {
        session,
    ...
    Ok(Router::new()
        ...
        .layer(
            // Kept even when mounted same-origin, since the standalone binary is still
            // reached cross-origin by a locally served page.
            CorsLayer::new()
                .allow_origin(Any)
                .allow_methods(Any)
