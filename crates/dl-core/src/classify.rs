...
pub enum MediaKind {
    ...
    HlsPlaylist,
    /// Vimeo's JSON adaptive manifest, which names its segments in JSON rather than in
    /// a playlist. Kept as its own kind because nothing else can read it: it is served
    /// as `application/json`, so every rule below would drop it, and the page holds no
    /// other address for a video restricted to Vimeo's own site.
    VimeoAdaptive,
}
...
pub fn classify(meta: &RequestMeta) -> Option<MediaCandidate> {
    ...
    if crate::sites::vimeo_adaptive::is_adaptive_playlist(&meta.url) {
        return Some(MediaCandidate {
            ...
            // The manifest states each segment's size, but the total is a property of the
            // rendition eventually chosen, not of the manifest.
            size: None,
        });
    }
...
fn derive_filename(
    ...
    let wanted_ext = match kind {
        // A playlist is not what lands on disk — an fMP4 is. The same is true of a
        // manifest: what is saved is the joined picture and sound.
        MediaKind::HlsPlaylist | MediaKind::VimeoAdaptive => "mp4".to_string(),
